    ...
    <div class="connexion">
        <form id="monFormulaire" action="traitement_modifier_recette.php" method="post">
            ...
            <div class="connect-p2">
                ...
                <input type="text" name="titre recette" id="titre_recette" value="<?php echo $reponse['titre'] ?>"
                    required>
                <label for="resume_de_la_recette"> Resume de la recette</label>
                <input type="text" name="resume_de_la_recette" id="resume_de_la_recette"
                    value="<?php echo $reponse['rec_resume'] ?>" required>
                <label for="lien_image"> lien de l'image</label>
                <input type="text" name="lien_image" id="lien_image" value="<?php echo $reponse['rec_image'] ?>"
                    required>
                <label for="contenu_de_la_recette"> Contenu de la recette</label>
                <input type="text" name="contenu_de_la_recette" id="contenu_de_la_recette"
                    value="<?php echo $reponse['contenu'] ?>" required>
                <label for="categorie">Choisissez la catégorie de votre recette : </label>
                <select name="categorie" id="categorie">
                    <option value=1 <?php if ($reponse['categorie'] == 1)
                        echo 'selected'; ?>>Entrée</option>
                    <option value=2 <?php if ($reponse['categorie'] == 2)
                        echo 'selected'; ?>>Plats</option>
                    <option value=3 <?php if ($reponse['categorie'] == 3)
